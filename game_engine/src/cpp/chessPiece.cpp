...
bool Pawn::isValidMove(int newRow, int newCol, GameState& game_state) const {
    ...
    if (newCol == col && newRow == row + rowDirection) {
        if (game_state.get_piece_at(newRow, newCol) == nullptr) {
            return true;
        }
    }
    ...
    if (newCol == col && newRow == row + 2 * rowDirection) {
        if ((color == Color::White && row == 6) || (color == Color::Black && row == 1)) {
            if (game_state.get_piece_at(row + rowDirection, newCol) == nullptr &&
                game_state.get_piece_at(newRow, newCol) == nullptr) {
                return true;
            }
        }
    }
    ...
    if (abs(newCol - col) == 1 && newRow == row + rowDirection) {
        Piece* target_piece = game_state.get_piece_at(newRow, newCol);
        if (target_piece != nullptr && target_piece->getColor() != color) {
            if (target_piece->getColor() == Color::White) {
                // delete piece
                game_state.remove_piece_from_board(newRow, newCol, game_state.wp);
            } else {
                game_state.remove_piece_from_board(newRow, newCol, game_state.bp);
            }
